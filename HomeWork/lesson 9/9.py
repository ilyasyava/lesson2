...
def dict_from_args(*args, **kwargs):
    ...
    if not all(isinstance(key, str) for key in kwargs):
        raise TypeError("Все аргументы-ключи должны быть строками")

    args_sum = sum(args)
    kwargs_max_len = max(len(key) for key in kwargs)
...
result = dict_from_args(2, 3, 5, name="Kirill", city="Minsk", country="Belarus")
print(result)